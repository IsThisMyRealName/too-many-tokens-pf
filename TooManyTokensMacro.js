let folderPath = "modules/too-many-tokens-pf/names.txt";
...
async function TooManyTokensMacro(selectedActor) {
  ...
  const subfolderPath = `modules/too-many-tokens-pf/Tokens/${actorName}`;
...
const createWildcardPath = (nameLists, checkedCheckboxes, actorName) => {
  let wildcardPath = `modules/too-many-tokens-pf/Tokens/${actorName}/`;
  nameLists.forEach((list, index) => {
    if (list.size > 0) {
      if (
